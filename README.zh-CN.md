...
<p align="center">
  ...
  更快输入 LaTeX，复用数学或 Markdown 模板，并通过内置 WASM 或可选本机 TeX 渲染 TikZ。
</p>
...
- 可选用自包含 WASM 渲染 TikZ 围栏代码块，支持实时预览和 SVG、PNG、JPEG、PDF 导出；需要更广兼容性时可使用本机 TeX；
...
**当前版本：v0.5.15。** 见 [CHANGELOG](CHANGELOG.md)。
...
| **单个公式导出** | 将选中或光标所在公式保存为矢量 SVG 或透明 PNG。 |
...
先在插件设置中开启 **公式导出与复制**（默认关闭）。关闭后会立即移除公式块和
公式面板的相关按钮，并隐藏导出命令及编辑器右键菜单入口。

| 控件 | 作用 |
| :--- | :--- |
| 复制图片（左侧） | 将透明 PNG 放入剪贴板，不弹出保存窗口。 |
| 下载（中间） | 打开格式菜单，选择 SVG 或 PNG 后再选择保存位置。 |
| 编辑源码（右侧，实时预览） | Obsidian 原生按钮，用于编辑公式源码。 |

鼠标停留在图片按钮上可查看说明，也可用键盘聚焦后按 Enter 或空格触发。
按钮始终针对所在的公式块。

将鼠标移到已渲染的行间公式块上，点击右上角的下载图标。在实时预览中，它位于原生
“编辑源码”按钮的紧邻左侧；阅读视图也提供相同菜单。

下载图标左侧的“复制公式图片”按钮使用 MathJax 复制透明 PNG。复制后可用
Ctrl+V（macOS 为 Cmd+V）粘贴到 PPT 或其他支持图片的应用，无需保存文件或安装 TeX。

- **导出当前公式为透明 PNG**：默认使用 Obsidian 自带的 MathJax，无需安装 TeX。
  输出 3 倍尺寸的透明图片，可直接插入 PPT 等文档。
- **本机 TeX 导出 SVG（更高质量）**：可选的矢量导出，适合在演示文稿中缩放使用。
  需要已安装的本机 TeX 及 SVG 转换工具（dvisvgm 或 pdftocairo）。
- **本机 TeX 导出 PNG（更高质量）**：使用同一个可选 TeX 渲染器输出透明 PNG。

MathJax 是默认方式。只有主动选择“本机 TeX”选项时，插件才会调用本机工具，
也不会自动安装工具。TeX 的字体和支持的命令可能与 MathJax 预览有所不同。

图片统一使用黑色公式和透明背景，不受笔记主题影响。PNG 适合快速复制和插入；
SVG 保留矢量路径，适合缩放。目前复制按钮仅提供 PNG，需要 SVG 时请使用下载。

剪贴板访问失败时，可下载 PNG 后在目标应用中插入文件。取消保存窗口不会导出文件。
本机 TeX 失败时，提示会显示可用的编译错误和源码位置；仍可单独选择 MathJax PNG。
复制与下载均不修改笔记。

公式面板、编辑器右键菜单和命令也提供 PNG（MathJax）及 SVG（本机 TeX）导出，
命令 ID 分别为 `export-current-formula-png` 和 `export-current-formula-svg`。
使用这些命令时，选中一个公式或将单个光标放到 Markdown 公式内；非空选区优先。
公式块按钮始终导出被点击的那个公式，不会移动光标或修改笔记。

导出跳过受保护的代码、前置元数据和 HTML 区域。笔记超过 100,000 个字符时，
请明确选中公式或使用公式块按钮。PNG 每条边最多 8192 像素，总像素数最多
1600 万。渲染或保存失败时会显示错误，不会写出不完整图像。
...
悬浮预览在首张图完成前显示源码，编辑过程中保留上一张成功图，不会被临时错误替换。
正文中的渲染块等待时只显示简短提示，首次渲染失败后才显示源码。
浮动图形完整适应窗口且不显示滚动条；阅读视图中的图形则按 SVG 内容高度自然撑开，
不会形成内部滚动区域。在桌面端，点击导出按钮会直接打开系统保存对话框；文件名
使用 `.svg`、`.png`、`.jpg`/`.jpeg` 或 `.pdf` 后缀即可选择对应格式。

阅读视图只会启动视口附近图形的渲染。完成的结果保存在有上限的内存缓存（最多 24 项 / 16 MiB）和持久缓存（最多 96 项 / 32 MiB）中，因此再次查看未改动的图形通常无需重新编译。面向打印的 Markdown 导出器会触发并等待全部 TikZ 代码块，包括从未滚动到视口中的代码块。设置页提供精简的诊断工具，可复制后端报告、清除缓存或重启渲染引擎，不增加状态栏项目。
...
在桌面端，将鼠标移到正文中已渲染的 TikZ 图形上，点击右上角的下载按钮，即可保存为
SVG、PNG、JPEG 或 PDF。在实时预览中，下载按钮位于原生编辑按钮旁；阅读视图和悬浮
预览窗口也提供下载入口。下载保存被点击图形的当前渲染结果，沿用实际生成该图的后端，
不会另行切换引擎、重新编译或展开源码；打印输出中不显示这些按钮。

- **内置 WASM（默认并推荐）：**使用 Math Chords 原创的 Rust 矢量核心，启动快，无需安装 TeX，也不会在运行时下载引擎，渲染工作避开主编辑线程。它在 Markdown 与打印导出中均保持为矢量 SVG。它是主要渲染器，后续 TikZ 语法会直接在这套核心中扩展。
- **本机 TeX（高级兼容模式）：**需要桌面端已安装 TeX，并因启动外部工具链而明显更慢。它适合 `pgfplots`、`circuitikz` 等宏包、文档专用宏和样式、完整 TeX 文本盒、特殊 OpenType/CJK 字体，以及必须与正式 TeX 编译结果一致的场景。普通图形会优先使用 DVI→SVG 路径，以便在 Markdown 与打印中保持清晰；生成 PDF 的引擎会在本机 `dvisvgm` 或同目录的 `pdftocairo` 能够完成转换时转为基于路径的 SVG，同时保留原始矢量 PDF 供直接导出。Math Chords 可从 TeX Live、MiKTeX、MacTeX、TinyTeX、Tectonic、PATH 或用户指定路径检测兼容引擎。
- **自动模式：**受支持的图形使用与手动 WASM 完全相同的实例和缓存；当能力检查确认某些语法无法忠实复现或 WASM 失败时，再选择本机 TeX。这样既保留快速路径，也避免返回看似可用但实际错误的图形。

内置的出版级子集覆盖理工科图形常见语法：笛卡尔与极坐标、有上限的数值宏和
`\foreach` 循环、相对与命名坐标、直线与贝塞尔曲线、圆、椭圆、矩形、网格、
闭合多边形、圆弧、有上限的函数图与坐标图、标准线型/颜色/透明度、Latex/Stealth
箭头、路径内标签，以及矩形或圆形节点。`pgfplots`、`circuitikz`、矩阵、图绘制、
calc 坐标、图案、装饰、裁剪、渐变、三维坐标和作用域变换等专用库或高级结构，
仍明确交给自动模式/本机 TeX，而不会由 WASM 静默近似。

这样的分工让普通用户获得快速、免安装的默认体验，同时把完整 TeX 生态作为明确的
专业出口保留下来。插件不会下载渲染引擎，也不会静默安装 TeX。切换后端后，所有已打开
Markdown 视图中的 TikZ 与当前编辑器预览都会立即刷新，无需逐个重新展开源码块。
...
粘贴事件提供的文本，以及明确触发的“复制公式图片”和“复制诊断报告”操作。
复制公式图片需要先开启默认关闭的 **公式导出与复制** 设置。插件不会在后台读取或
监控剪贴板内容。由于这些复制操作会写入系统剪贴板，审查工具可能显示 **Clipboard
Access（剪贴板访问）** 能力提示。
...
**内置命令**（可在 **设置 → 快捷键** 中绑定或修改）：**Open formula panel**、**Insert inline math**、**Insert display math**、**Wrap display math with environment**、**Convert LaTeX Delimiters in Selection**、**Convert LaTeX Delimiters in Current File**、**本机 TeX 导出 SVG（更高质量）**、**导出当前公式为透明 PNG**。
