...
**当前版本：v0.5.14。** 见 [CHANGELOG](CHANGELOG.md)。
...
预览窗口也提供下载入口。下载针对点击的图形，不会展开源码；打印输出中不显示这些按钮。
...
- **本机 TeX（高级兼容模式）：**需要桌面端已安装 TeX，并因启动外部工具链而明显更慢。它适合 `pgfplots`、`circuitikz` 等宏包、文档专用宏和样式、完整 TeX 文本盒、特殊 OpenType/CJK 字体，以及必须与正式 TeX 编译结果一致的场景。普通图形会优先使用 DVI→SVG 路径，以便在 Markdown 与打印中保持清晰；生成 PDF 的引擎会在本机 `dvisvgm` 支持 PDF 输入时转为基于路径的 SVG，同时保留原始矢量 PDF 供直接导出。Math Chords 可从 TeX Live、MiKTeX、MacTeX、TinyTeX、Tectonic、PATH 或用户指定路径检测兼容引擎。
