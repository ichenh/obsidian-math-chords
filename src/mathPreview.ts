...
function nodeAsElement(node: Node | null): HTMLElement | null {
  if (!node) return null;
...
function findRenderedMathRect(view: EditorView): DOMRect | null {
  ...
  let element = nodeAsElement(selection.anchorNode);
...
class InlinePreviewLayer {
  ...
  private adaptSize(): void {
    ...
    this.panel.toggleClass("is-scroll-x", needsScroll);
    if (needsScroll) {
      this.panel.setCssProps({ "--mc-preview-max-width": `${maxWidth}px` });
    }

    const panelWidth = Math.min(contentWidth, maxWidth);
    const left = Math.max(8, Math.min(anchor.left, win.innerWidth - panelWidth - 8));
    this.host.setCssProps({ "--mc-preview-left": `${left}px` });
  }
  ...
  private isMathVisible(): boolean {
    const mjx = this.body.querySelector(".mjx-container");
